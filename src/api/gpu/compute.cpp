...
extension_set::extension_set(const std::string &extensions) { add_extensions(extensions); }
    ...
    for (auto &platform : platforms) {
        ...
        for (const auto &device : devices) {
            ...
            if (required_extensions.is_subset_of(device_extensions)) {
                ...
                LOG("Found GPU device '%s', but some required extensions are not "
                    "supported.\n",
...
command_queue init(const os::window &window)
{
    ...
    cl_context_properties context_properties[] = { CL_CONTEXT_PLATFORM,
                                                   reinterpret_cast<cl_context_properties>(c_platform_id),
                                                   CL_GL_CONTEXT_KHR,
                                                   reinterpret_cast<cl_context_properties>(
                                                       window.get_graphics_context()),
                                                   display_property,
                                                   display_handle,
                                                   0 };
